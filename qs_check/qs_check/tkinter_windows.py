...
from .solman_api import SolutionManager
...
class Application(Window):
    ...
    def set_cd(self):
        ...
        if self.request.is_exist() is True:
            ...
            pswd = self.credential.password
            chrome_driver_path = self.config.path
            cd = self.request.number
            self.solman = SolutionManager(driver_path=chrome_driver_path, username=user, password=pswd, sol_profile=True)
            chg_request = self.solman.change_request(request_number=cd)
